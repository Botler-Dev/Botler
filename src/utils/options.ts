...
type AdditionalOptionsDefinition<OptionsType extends object, Additional extends object> =
  {
    [RequiredKeys in keyof Omit<Additional,
      Extract<keyof Additional, keyof OptionsType>>]-?: OptionValue<Additional, RequiredKeys>;
  } & {
 ...
 */
export default abstract class Options<OptionsType extends object> {
  /**
   * Raw state of the options. Use .set() if you want to change the options.
   *
   * @protected
  ...
  /**
   * If an extended class adds options use this method to register them.
   * They can also overwrite existing options.
   *
   * @protected
   * @template AdditionalOptionsType Type of the new options extending the original
   * @param {AdditionalOptionsDefinition<OptionsType,
   * AdditionalOptionsType>} options Additional options definition
   * @memberof Options
   */
  protected addOptions<AdditionalOptionsType extends object>(
    options: AdditionalOptionsDefinition<OptionsType, AdditionalOptionsType>,
  ) {
   ...
   * @memberof Options
   */
  private addOption<Key extends keyof OptionsType>(
    name: Key,
    value: OptionValue<OptionsType, Key>,
  ) {
    Object.defineProperty(this, name, {
      get: typeof value === 'function' ? value as () => any : () => this.options[name] || value,
      set: (v) => { this.options[name] = v; },
      configurable: true,
    });
  }
   ...
   * This assignment is on a per key basis in the root so won't overwrite existing values
   * not defined in the provided options.
   *
   * @protected
   * @template AdditionalOptionsType Type of the new options extending the original
   ...
   * @memberof Options
   */
  protected set<AdditionalOptionsType extends object>(
    options: Partial<OptionsType & AdditionalOptionsType>,
  ) {
