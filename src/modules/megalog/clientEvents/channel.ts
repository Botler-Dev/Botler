...
export function registerChannelClientEventListeners(utils: MegalogClientEventUtils): void {
  utils.listenToGuildEvent('channelCreate', channelToGuild, channelToMatchFilter('CHANNEL_CREATE'));

  utils.listenToGuildEvent('channelDelete', channelToGuild, channelToMatchFilter('CHANNEL_DELETE'));

  utils.listenToGuildEvent('channelUpdate', channelToGuild, async (oldChannel, newChannel) => {
  ...
  utils.listenToGuildEvent('channelPinsUpdate', channelToGuild, async channel => {
    if (!(channel instanceof GuildChannel)) return undefined;
    return {
      checker: entry =>
  ...
  utils.listenToGuildEvent('webhookUpdate', channelToGuild, async channel => ({
