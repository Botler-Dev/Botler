...
export function registerMemberClientEventListeners(utils: MegalogClientEventUtils): void {
  ...
  utils.listenToGuildEvent('guildMemberRemove', memberToGuild, async ({id: memberId}) => ({
    checker: entry =>
      (entry.action === 'MEMBER_BAN_ADD' || entry.action === 'MEMBER_KICK') &&
      checkAuditLogEntryTargetId(entry, memberId),
  }));

  utils.listenToGuildEvent('guildMemberUpdate', memberToGuild, async (oldMember, newMember) => ({
    checker: entry => {
